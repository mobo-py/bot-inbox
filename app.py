...
emailtypes = ["Complaint", "Request", "Update", "Reminder", "Question", "Follow-up", "Other"]
...
userchoice = int(input("Choose the type of email by entering the corresponding number: ")) - 1

emailtype = emailtypes[userchoice]

if emailtype == "Other":
    emailtype = input("Enter the type of email:  ")





emailthemes = ["Professional", "Casual", "Friendly", "Formal", "Concise", "Other"]

for i in emailthemes:
    print(f"{emailthemes.index(i)+1}. {i}")

themechoice = int(input("Choose a tone by entering the corresponding number: ")) - 1

usertheme = emailthemes[themechoice]

if usertheme == "Other":
    usertheme = input("Enter the tone of the email:  ")
...
def generateresponse(username, emailtype, usertheme, recipientname, title, userprompt):
    prompt = f"""
...
print(generateresponse(username, emailtype, usertheme, recipientname, title, userprompt)['message']['content'])